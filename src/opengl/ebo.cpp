...
EBO::EBO(const std::vector<uint8>& indices) noexcept {
	glGenBuffers(1, &this->id);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, this->id);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, static_cast<GLsizei>(indices.size() * sizeof(uint8)), indices.data(), GL_STATIC_DRAW);
}
