...
struct BoundingBox {
	// Immutable AABB in the model's local space.
	// This is the base of the bounding box

	// Minimum corner of the bounding box in local space
	vec3<float> local_min = vec3<float>(FLT_MAX);
	// Maximum corner of the bounding box in local space
	vec3<float> local_max = vec3<float>(-FLT_MAX);

	// AABB in world space, updated by the model matrix.
	...
	inline vec3<float> get_center_position() const noexcept {
		return this->min + (this->get_size() * 0.5f);
	}
	...
	void draw_local_bounds(const Camera& camera, const Color& color = Colors::RED, const bool stripped = false) const noexcept;
	// Draws the bounding box of the model in world space.
	// This is only intented to be used as a debug tool
	void draw_world_bounds(const Camera& camera, const Color& color = Colors::RED, const bool stripped = false) const noexcept;
	...
	private:
		void draw(const bool world, const Camera& camera, const Color& color, const bool stripped) const noexcept;
};
