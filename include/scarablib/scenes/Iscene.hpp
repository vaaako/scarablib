...
class IScene {
	...
	public:
		...
		template <typename U>
		std::shared_ptr<U> get_by_key(const char* key) noexcept {
			...
			if(!casted_ptr) {
				throw ScarabError("Failed to cast object with key '%s' to type", key);
			}
		...
		void remove_by_key(const char* key) noexcept;
...
template <typename T>
void IScene<T>::remove_by_key(const char* key) noexcept {
	auto it = this->scene.find(key);
	if(it == this->scene.end()) {
		throw ScarabError("Key '%s' was not found", key);
	}
	this->scene.erase(it);
}
