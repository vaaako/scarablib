...
class Mesh {
	public:
		...
		Mesh(const GLuint& vao_id, const std::vector<Vertex>& vertices, const std::vector<uint32>& indices) noexcept;
		~Mesh() noexcept;
		...
		void make_buffers(const std::vector<Vertex>& vertices, const std::vector<uint32>& indices) noexcept;

		// Generate VAO, VBO and EBO to a existing VAO.
		// This is used for custom models
		void make_buffers(const GLuint& vao_id, const std::vector<Vertex>& vertices, const std::vector<uint32>& indices) noexcept;
		...
		virtual inline const VAO& get_vao() const noexcept {
			return *this->vao;
		}
