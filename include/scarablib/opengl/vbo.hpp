...
class VBO {
	public:
		...
		void alloc_data(const uint32 size, const void* data, const GLenum usage) const;

		// Allocate and initialize the data store for the VBO using a vector of float
		void alloc_data(const std::vector<float>& data, const GLenum usage) const;
		...
		void link_attrib(const uint32 index, const uint32 size, const uint32 total_byte_size, const uint32 offset) const;

		// Automatically make the VBO from a vector of Vertex.
		// Don't use this with any other creation method
		void make_from_vertex(const std::vector<Vertex>& data, const uint32 size, const GLenum usage) const;

		// Automatically make the VBO from a vector of vertices coords.
		// Don't use this with any other creation method
		void make_from_vertices(const std::vector<float>& data, const uint32 size, const GLenum usatge) const;

		// Updates the the data of the VBO
		void update_data(const uint32 size, const void* data, const GLenum usage) const;
